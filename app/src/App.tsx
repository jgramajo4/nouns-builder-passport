...
export default function App() {
  ...
  const { data: milestones = [], isLoading: msLoading }  = useMilestones(address)

  const statusBar: React.CSSProperties = {
    background: '#1E1A0A', borderTop: '1px solid #3A3020',
    padding: '2px 8px', fontSize: 9, color: '#6A5A38',
    display: 'flex', justifyContent: 'space-between',
    fontFamily: 'IBM Plex Mono, monospace',
  }

  return (
    <div style={{ background: '#1A1610', minHeight: '100vh', fontFamily: 'IBM Plex Mono, monospace' }}>

      {/* always-visible passport card */}
      <PassportCard passport={passport} isLoading={passportLoading} />

      {/* CDE panel */}
      <div style={{ background: '#242018', border: '1px solid #3A3020', margin: '0' }}>
        ...
        <div style={{ background: '#1A1610', padding: 8 }}>
          ...
          {tab === 'share'     && <ShareTab passport={passport} milestones={milestones} />}
        </div>

        <div style={statusBar}>
          <span>Schema 1 · Schema 2 · Schema 3 · NounsPassportResolver · NounHolderResolver</span>
          <span>⌐◨-◨ EAS Mainnet · Block #{typeof window !== 'undefined' ? '…' : '—'}</span>
        </div>
      </div>
