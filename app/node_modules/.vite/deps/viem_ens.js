import {
  TypedData_exports
} from "./chunk-BWJZRI63.js";
import {
  AbiError_exports
} from "./chunk-TEPWYFYD.js";
import {
  import_index
...
var bls12_381 = bls({
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      const L = Fp.BYTES;
      const slc = (b, from21, to2) => bytesToNumberBE(b.slice(from21, to2));
...
function fromCreate(options) {
  const from21 = fromHex(from3(options.from));
  let nonce = fromNumber(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from3(`0x${keccak256(fromBytes2([from21, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from3(slice2(keccak256(concat(fromHex("0xff"), from21, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function normalize(fn) {
  afn(fn);
  return { encode: (from21) => from21, decode: (to2) => fn(to2) };
}
function convertRadix(data, from21, to2) {
  if (from21 < 2)
...
var radix2carry = (from21, to2) => from21 + (to2 - gcd(from21, to2));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from21, to2, padding2) {
  aArr(data);
  if (from21 <= 0 || from21 > 32)
    throw new Error(`convertRadix2: wrong from=${from21}`);
  if (to2 <= 0 || to2 > 32)
    throw new Error(`convertRadix2: wrong to=${to2}`);
  if (radix2carry(from21, to2) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from21} to=${to2} carryBits=${radix2carry(from21, to2)}`);
  }
  let carry = 0;
  let pos = 0;
  ...
  for (const n of data) {
    anumber2(n);
    if (n >= max)
      throw new Error(`convertRadix2: invalid data word=${n} from=${from21}`);
    carry = carry << from21 | n;
    if (pos + from21 > 32)
...
var base32crockford = chain(radix2(5), alphabet3("0123456789ABCDEFGHJKMNPQRSTVWXYZ"), join(""), normalize((s) => s.toUpperCase().replace(/O/g, "0").replace(/[IL]/g, "1")));
...
var hex = hasHexBuiltin ? hexBuiltin : chain(radix2(4), alphabet3("0123456789abcdef"), join(""), normalize((s) => {
...
function from12(response, options = {}) {
...
function from13(provider, _options = {}) {
  if (!provider)
